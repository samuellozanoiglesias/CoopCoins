...
def make_train_EASY(config):
    ...
    # === LOSS FUNCTION ===
    @jit
    def compute_loss(model, obs, actions, advantages, returns, value_coef, entropy_coef, clip_eps, old_log_probs, old_values):
    ...
    def train_minibatches(model, opt_state, obs_batch, action_batch, advantage, return_batch, old_value_batch, key, optimizer, minibatch_size, epochs, clip_eps, old_log_probs):
        ...
        def compute_loss_batch(model, obs, actions, adv, returns, old_values):
            def single_loss(m, o, a, ad, r, ov):
                return compute_loss(m, o, a, ad, r, config["VF_COEF"], config["ENT_COEF"], clip_eps, old_log_probs, ov)
            losses = jax.vmap(single_loss, in_axes=(None, 0, 0, 0, 0, 0))(model, obs, actions, adv, returns, old_values)
            return losses.mean()
        ...
        def epoch_step(carry, _):
            ...
            def minibatch_step(carry, i):
                ...
                batch_old_values = old_value_batch[idx]

                loss, grads = value_and_grad(compute_loss_batch)(model, batch_obs, batch_actions, batch_adv, batch_returns, batch_old_values)
